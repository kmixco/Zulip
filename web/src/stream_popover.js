...
export async function build_move_topic_to_stream_popover(
    ...
    function caseInsensitiveInlucde(topics, current_topic) {
        for (const topic of topics) {
            if (topic.toUpperCase() === current_topic.toUpperCase()) {
                return true;
            }
        }
        return false;
    ...
    function update_warning_banner() {
        ...
        if (stream_widget_value !== undefined) {
            ...
            if (!caseInsensitiveInlucde(stream_topics, new_topic_name.trim())) {
                $("#move_topic_modal .move_topic_form_warning_container").html("");
                return;
            }
        }
