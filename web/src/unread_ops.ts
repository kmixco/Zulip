...
const update_flags_for_narrow_response_schema = z.object({
    ...
    skipped_marking_unread_stream_ids: z.array(z.number()),
});
...
function handle_skipped_unsubscribed_streams(skipped_marking_unread_stream_ids: number[]): void {
    if (skipped_marking_unread_stream_ids.length) {
        ...
        const title_text = $t({defaultMessage: "Skipped unsubscribed streams"});
...
function do_mark_unread_by_narrow(
    ...
    void channel.post({
        ...
        success(raw_data) {
            ...
            const skipped_marking_unread_stream_ids = data.skipped_marking_unread_stream_ids;
            if (!data.found_newest) {
                assert(data.last_processed_id !== null);
            ...
            if (skipped_marking_unread_stream_ids.length) {
                handle_skipped_unsubscribed_streams(skipped_marking_unread_stream_ids);
            }
        },
        error(xhr) {
