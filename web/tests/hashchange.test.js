...
run_test("update_hash_to_match_filter", () => {
    ...
    let terms = [
        {operator: "channel", operand: "devel", negated: false},
    ...
    echo.update_topic_hash_to_contain_with_term({id: 12, stream: "devel", topic: "test"});
    assert.deepEqual(
        [...terms, {operator: "with", operand: "12"}],
        message_lists.current.data.filter.terms(),
    );
    assert.equal(window.location.hash, "#narrow/stream/devel/topic/test/with/12");

    // Message does not belongs to the same channel and topic as of current narrow.
    message_lists.current.data.filter = new Filter(terms);
    echo.update_topic_hash_to_contain_with_term({id: 12, stream: "devel", topic: "test 2"});
